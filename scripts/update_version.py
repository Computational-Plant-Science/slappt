...
def update_docs_conf_py(version: Version = None):
    lines = _docs_conf_py_path.read_text().rstrip().split("\n")
    with open(_docs_conf_py_path, "w") as f:
        for line in lines:
            line = f"release = '{version}'" if "release = " in line else line
            f.write(f"{line}\n")

    print(f"Updated {_docs_conf_py_path} to version {version}")
